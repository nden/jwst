...
and observing modes by applying various science corrections squentially,
...
hosted open-source on Github, and can be run either via command line interface
(`strun`) or via the Python interface.
...
to produce output products at different calibration levels:
	...
	:Stage 1: Detector-level corrections and ramp fitting for individuall
			  exposures.
...
Each pipeline stage consists of a series of sequential steps (i.e, saturation
...
output intermediate output products can be controlled. 
...
The following will be a brief overview of how the pipeline code in `jwst` is
organized.

There are two options for running the JWST Pipeline: the Python interface, and
the command line interface called `strun`. Both of these access the same
underlying code.


**Pipeline and Step Classes** 

The JWST pipeline is organized into two main classes - the `pipeline` class and
the `step` class. Pipelines are made up of sequential `step` classes, but both
are standalone objects that are designed to be configured and run on input data.
...
Within the `jwst` repository, there are seperate modules for each pipeline step. 
...
JWST data, WCS infromation is encoded in an ASDF extension in the FITS file that
...
`pipeline` and `step`, and commad line tools that are shared between the JWST
and Roman pipelines.
