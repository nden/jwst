...
class ResampleSpecData(ResampleData):
    ...
    def wcs_new_miri_lrs(self, refmodel=None):
        pass
    def build_interpolated_output_wcs(self, refmodel=None):
        ...
        for im, model in enumerate(self.input_models):
            dm = isinstance(self.input_models[0], datamodels.DataModel)
            log.info(f"input_models[0], {self.input_models[0]}, {dm}")
            wcs = model.meta.wcs
            ...
            #wavelength_array = lam[~np.isnan(lam)]
            ...
            if im == 0:
                ...
                log.debug(f"lam_center_index1, {lam_center_index}")
                #lam_center_index = int(zero_point[1])
                # log.debug(f"lam_center_index2 used, {lam_center_index}")
                ...
                # # log.debug(f"ra, dec, lam center pt old, {ra_center_pt, dec_center_pt}")
                ...
                #
                #
                # ra_center_pt, dec_center_pt, lam_center_pt = model.meta.wcs(*zero_point)
                # log.debug(f"ra, dec, lam center pt new, {ra_center_pt, dec_center_pt}")
                ...
                # fit_model = Linear1D()
                # log.info(f"x_tan_array.shape, {x_tan_array.shape}, {y_tan_array.shape}, {self.pscale_ratio}")
                ...
                # xt, yt = (Shift(-23) & Shift(-294.2) | pix_to_xtan & pix_to_ytan | undist2sky1).inverse(model.meta.target.ra, model.meta.target.dec)
                # log.debug(f"xt, yt {xt, yt}")
                # log.debug(f"pix_to_xtan {pix_to_xtan}")
                # log.debug(f"pix_to_ytan {pix_to_ytan}")
